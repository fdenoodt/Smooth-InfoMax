...
Machine learning considers the problem of programming a computer via data, rather than explicit code instructions. In particular, in supervised machine learning, we may be interested in finding a mapping function $f(\cdot)$ that can infer a label $\vecti{y} \in \mathcal{Y}$ given an observation or \textit{feature vector}  $\vecti{x} \in \X$. The function $f(\cdot)$ can then be denoted as follows:
$$
...
$$\sample{\vect{z}}{\qvae} \quad \text{which is also written as: } \quad \sample{\vect{z}}{\qvaeBlank}$$
