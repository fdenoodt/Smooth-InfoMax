...
\section{Motivation} %\section{Problem setting}
	...
	% repr learn autoenc + vae (disentenglement)
		...
		%For instance, if a classifier may require learning the background colour of an image from the latent representation obtained from a $\beta$-VAE. It would be advantageous if the information for the background colour were concentrated in a single, non-influential unit component, separate from other factors like the colour of the face. % TODO: I could reformulate this, and mention meta priors such as in this paper: https://arxiv.org/pdf/1812.05069.pdf
...
\section{The learning objective}
	...
	% 1) cont space: arguments: all points are at center, have little space, meanwhile latents form entire neighboorhoud. resulting in optimally use all space, resuling in continuous space around origin.
	% 2) smooth transitions: MI(x, z) is maximised for entire region. rapid changes in generative factors are thus punished by having z's in an entire neighboorhood. so traveling around in neighbourhood doesn't allow for rapid changes in the latent space.
...
\section{Computational benefits}
	...
	\subsubsection{Interpretability of final and intermediate modules}
		...
		% Without regularisation, the shape of the latent space is unpredictable, and thus we do not known what from which data points samples are taken when choosing a random encoding, or when interpolating between two well formed encodings representations (aka, two encodings obtained by encoding data points from the dataset).
